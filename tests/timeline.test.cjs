...
test('compact clusters retain every monument exactly once and keep the country and political lanes', () => {
  ...
  assert.equal(ids.length, SITES.length);
  assert.equal(new Set(ids).size, SITES.length);
  ...
  assert.equal(timeline.lane(SITES.find(site => site.id === 'nx_xixialing'), DYN), 'north');
  assert.equal(timeline.lane(SITES.find(site => site.id === 'nx_108towers'), DYN), 'north');
