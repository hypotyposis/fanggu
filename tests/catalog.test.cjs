...
test('all 203 catalogue entries have a real PNG, matching dimensions and a map location', () => {
  assert.equal(SITES.length, 203); assert.equal(new Set(SITES.map(s => s.id )).size, 203);
