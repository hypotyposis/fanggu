...
本轮新增两处，增补分支完成时山西共72处、全库203处。两处默认未到访，不预填日期，已有浏览器记录优先；没有修改个人存储格式或既有古迹 ID。下面制作与检查记录对应本轮分支，合回主工作区后的检查另见文末。
...
## 合回 main 的检查（2026-09-18）

用户明确要求提交并合回 main。山西增补提交 `5a7d53c` 已快进进入 main；恢复主工作区已有改动时解决重叠冲突，其他任务的改动仍保留为未提交、未暂存状态，没有随山西提交一并提交。自动备份 `9cde0da` 保留用于恢复，不再次应用到已恢复的工作区。

合并时主工作区已有其他批次增补，本次检查的实际目录为279处、队列276张，山西仍为72处；这不是山西单批新增数量。恢复后核对原有277处古迹正文、274条队列记录、277张设色清单记录及无重叠文件哈希，全部与合并前一致。

按主工作区当前原型流程重建线稿及来源页，再转码并汇总设色：279张交付可用，原有277张设色缓存全部复用，两张新增图原件保持不变、继续待用户验收。合并后 `node --test --test-reporter=dot tests/*.test.cjs` 的116项测试和 `python3 -B tests/test_transparency.py` 的14项测试通过；`git diff --check` 通过，没有残留冲突或暂存改动。

真实浏览器在主目录独立预览来源重新检查山西／照壁／“影壁”筛选、“烈石神祠”搜索、两处详情图片实际加载、Back 筛选恢复，以及两张深色线稿／透明设色对照页；没有控制台错误。未重新验证手机实机触摸。仅合入本地 main，未推送远端或发布公网。
