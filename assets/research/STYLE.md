...
高句丽使用石褐 `--goguryeo`，渤海使用灰绿 `--balhae`，与北魏、唐分别分类；具体色值以 `style.css` 为准。所属时期依据逐图研究记录，不能以同年代中原朝代替代。
...
西夏独立使用沙褐 `--sand: #c6ac85`，不并入宋或辽金。时代范围1038—1227年参见 [UNESCO西夏陵说明](https://whc.unesco.org/en/list/1736)。遗址按现存形态绘制，不依据复原模型补造外装；塔群的始建说、国保名录断代与后世重修分别说明。
