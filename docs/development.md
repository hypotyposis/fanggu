...
| `library.js` | 个人记录、校验、迁移、持久化和备份合并；可注入存储用于测试 |
| `journal.js` | 首页与详情页共用表单、心愿和到访动作、跨标签页同步 |
...
`library.js` 使用 `localStorage` 键 `fanggu.library.v1`，当前值为版本 2；后缀 `v1` 不是当前备份版本。最小版本 2 结构示例：
...
- 导出额外带 `exportedAt`；导入支持版本 1、2，同 ID 以备份值覆盖，其余记录保留。格式和日期校验失败不能改写原存储。
- 保存前读取最新数据，避免另一标签页的笔记被旧状态覆盖；`journal.js` 监听存储变化以刷新显示。
- 无法解析的原始内容仍可导出，存储失败须显示错误。迁移不能通过清空存储、换键或重置默认值实现。

`sites.js` 的维护者正文允许现有受控 HTML；用户笔记、导入内容和搜索词必须经文本节点或 `textContent` 展示。不要混用这两种内容来源。
...
| 改动 | 重点测试 | 额外检查 |
| --- | --- | --- |
| 个人记录、迁移、备份 | `tests/library.test.cjs`、`tests/facets.test.cjs` | 导出导入、刷新、多标签页和存储失败 |
...
5. 编辑日期、笔记并刷新，导出后修改一个测试记录再导入，确认同 ID 备份覆盖而其他记录保留。打开两个标签页检查新笔记不会被旧状态覆盖。
