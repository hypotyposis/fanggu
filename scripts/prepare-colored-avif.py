...
def prepare_source(original, id, source_hash, legacy_source_hash=None, preparation=None):
...
def convert(id, source, source_hash, old, published=None, preparation=None):
    src = ROOT / source
    ...
    if (old.get("backgroundPreparation") == preparation
            ...
            and old.get("alpha", {}).get("min") == 0 and old.get("alpha", {}).get("max") == 255):
        return id, old, True
    ...
    with Image.open(src) as original:
        ...
        image, report = prepare_source(original, id, source_hash, legacy_hash, preparation)
        if report.get("method") == "white-matte-v1":
            if not report.get("interiorRgbUnchanged"):
                raise ValueError(f"Interior RGB changed during white-background extraction: {id}")
        elif not np.array_equal(np.asarray(image)[:, :, :3], np.asarray(original.convert("RGB"))):
    ...
    with tempfile.TemporaryDirectory(prefix=f".{id}-", dir=OUTPUT) as directory:
        ...
        with Image.open(staged) as decoded:
            decoded.load()
            if decoded.size != image.size or decoded.mode != "RGBA" or decoded.getextrema()[3] != (0, 255):
...
def main():
    ...
    for item in queue["entries"]:
        metadata = json.loads((ROOT / item["record"]).read_text())
        if metadata.get("background_preparation"):
            preparations[item["id"]] = {**metadata["background_preparation"], "processorSha256": digest(WHITE_EXTRACTOR)}
    ...
    with ProcessPoolExecutor(max_workers=4) as pool:
        futures = {pool.submit(convert, id, source, hashes[id], cached.get(id, {}), published.get("images", {}).get(id), preparations.get(id)): id for id, source in sources}
