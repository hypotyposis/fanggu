...
(function (root, factory) {
  ...
  const empty = () => ({ version: 2, customSites: [], records: {}, links: {} });
  ...
  function create(catalog, storage) {
    ...
    function validate(input) {
      if (!plain(input) || ![1, 2].includes(input.version) || !Array.isArray(input.customSites) || !plain(input.records) || (input.version === 2 && !plain(input.links))) throw new Error('这不是可识别的访古备份');
      if (input.customSites.length > 2000 || Object.keys(input.records).length > 5000) throw new Error('备份条目过多');
    ...
    return {
      all, record, today, error: () => loadError,
  ...
  return { KEY, create, today, validDate };
});
